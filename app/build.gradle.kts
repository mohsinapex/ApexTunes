...
val baseApplicationId = "com.mohsinraza.mohsinapexmusic"
val applicationIdOverride = System.getenv("APEXTUNES_APPLICATION_ID")?.takeIf { it.isNotBlank() }
    ?: System.getenv("METROLIST_APPLICATION_ID")?.takeIf { it.isNotBlank() }
val appNameOverride = System.getenv("APEXTUNES_APP_NAME")?.takeIf { it.isNotBlank() }
    ?: System.getenv("METROLIST_APP_NAME")?.takeIf { it.isNotBlank() }
val debugKeystorePathOverride = System.getenv("APEXTUNES_DEBUG_KEYSTORE_PATH")?.takeIf { it.isNotBlank() }
    ?: System.getenv("METROLIST_DEBUG_KEYSTORE_PATH")?.takeIf { it.isNotBlank() }
val debugKeystorePassword = System.getenv("APEXTUNES_DEBUG_KEYSTORE_PASSWORD")?.takeIf { it.isNotBlank() }
    ?: System.getenv("METROLIST_DEBUG_KEYSTORE_PASSWORD")?.takeIf { it.isNotBlank() } ?: "android"
val debugKeyAlias = System.getenv("APEXTUNES_DEBUG_KEY_ALIAS")?.takeIf { it.isNotBlank() }
    ?: System.getenv("METROLIST_DEBUG_KEY_ALIAS")?.takeIf { it.isNotBlank() } ?: "androiddebugkey"
val debugKeyPassword = System.getenv("APEXTUNES_DEBUG_KEY_PASSWORD")?.takeIf { it.isNotBlank() }
    ?: System.getenv("METROLIST_DEBUG_KEY_PASSWORD")?.takeIf { it.isNotBlank() } ?: "android"
...
android {
    namespace = "com.mohsinraza.mohsinapexmusic.music"
    compileSdk = 37
    ...
    buildTypes {
        ...
        debug {
            ...
            if (appNameOverride == null) {
                resValue("string", "app_name", "ApexTunes")
            }
            signingConfig =
    ...
    androidComponents {
        onVariants { variant ->
            variant.outputs.forEach { output ->
                output.outputFileName.set("ApexTunes.apk")
            }
        }
    }
